...
public class VitalSigns extends AppCompatActivity {
    ...
    ArrayList<String>  vitalSignname;
    ArrayList<String>  unitname;
    Connectiondetector connectiondetector;

EditText editText,search_item;
Button button;
   ...
   // @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                if (strText.length() != 0) {
                    ...
                    for (int i = 0; i < vitalsignlists.size(); i++) {
                        if (vitalsignlists.get(i).getSign().toLowerCase().startsWith(strText.toLowerCase())) {
                            vitalsignlistsTemp.add(vitalsignlists.get(i));
                        }
                    }
                    customAdapter_vitalsigns= new CustomAdapter_vitalsigns(vitalsignlistsTemp,context);
                    recyclerView.setAdapter(customAdapter_vitalsigns);
                    customAdapter_vitalsigns.notifyDataSetChanged();
                } else {
                    Toast.makeText(VitalSigns.this,"No match found",Toast.LENGTH_LONG).show();
    ...
    private Request listVitalSign()
    {
        ...
        return new Request.Builder()
                ...
                .url(ApiUtils.BASE_URL2+"newlistVitalSign")
                .post(body)
                .build();
    }
    ...
    private Request AddVitalSignApp()
    {
        ...
        return new Request.Builder()
                .addHeader("X-Api-Key", "AB5433GMDF657VBB")
                .addHeader("Content-Type", "application/json")
                .url(ApiUtils.BASE_URL2+"AddVitalSign")
                .post(body)
                .build();
    }
